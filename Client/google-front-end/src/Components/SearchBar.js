...
export default function SearchBar() {
	...
	const [expand, setExpandValue] = useState(false);

	const onClickExpand = () => {
		!expand ? setExpandValue(true) : setExpandValue(false);
	};

	return (
		// <form class="searchOne">
		<div
			onClick={onClickExpand}
			className={`${expand ? "searchOneClicked" : "searchOne"}`}
			// className="searchOne"
		>
			<svg
				aria-hidden="true"
			...
			<input
				onClick={onClickExpand}
				// className="search"
				className={`${expand ? "searchExpand" : "search"}`}
				type="text"
